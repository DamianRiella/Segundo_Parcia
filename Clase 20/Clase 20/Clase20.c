...
*/
void person_setLastName(Person* this, int lastName);
{
    if(lastName[0]!='\0')
    {
        strncpy(this->)
...
int parseData( char* fileName, Person* arrayPersonas ){
    ...
    do{
        ...
        if(r==4){
            auxId=atoi(var1);
            person_set(arrayPersonas[i],auxId);

            person_setName(arrayPersonas[i],auxName);
            person_setLastName(arrayPersonas[i],auxLastName);

            auxAge=atoi(var4);
            person_setAge(arrayPersonas[i],auxAge);
